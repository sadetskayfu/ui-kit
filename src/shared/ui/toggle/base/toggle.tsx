...
export const Toggle = React.forwardRef(
	(props: Toggle.Props, forwardedRef: React.ForwardedRef<HTMLButtonElement>) => {
        ...
        const isPressed = pressed ?? (groupValue !== undefined && (Array.isArray(groupValue) ? groupValue.includes(value) : groupValue === value))
        const isDisabled = disabled ?? groupContext?.disabled ?? false

        const state: Toggle.State = React.useMemo(() => ({ pressed: isPressed, disabled: isDisabled }), [isDisabled, isPressed])
		...
		const element = useRenderElement('button', {
            ...
            state,
			ref: forwardedRef,
			props: [{ 'aria-pressed': isPressed, disabled: isDisabled, onClick: handleClick }, otherProps],
		});
